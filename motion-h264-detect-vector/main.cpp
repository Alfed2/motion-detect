...
uint8_t* config_keyframe;
int config_keyframe_size=0;
...
static void Init_MP4(){
	...
	AVPacket opkt1;
        av_init_packet(&opkt1);

        opkt1.data=(uint8_t* )av_mallocz(config_keyframe_size);
        memcpy(opkt1.data,config_keyframe,config_keyframe_size);
        opkt1.size=config_keyframe_size;

        opkt1.pts = opkt1.dts = 1;
        opkt1.pts = av_rescale_q(opkt1.pts, omxtimebase, time_base);
        opkt1.dts = av_rescale_q(opkt1.dts, omxtimebase, time_base);
        opkt1.duration = av_rescale_q(1, omxtimebase, time_base);
        int ret1 = av_write_frame(outputFormatCtx, &opkt1);
        if(ret1 < 0)  fprintf(stderr, "Error write frame");
...
void camera_video_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
    ...
    if (number_of_sequence==35 && count_exceeding==0 ) {
       AlertSendVideo = true;
       if (output_buffer) {
         unsigned char* pointer = (unsigned char *)(output_buffer->data);
...
static void encoder_output_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
	...
	for (int i=0; i < num_mv; i++) {
        ...
        number_of_changes = vcount;    
        //fprintf(stderr, " Vector sum %d\n  FPS %0.1f",vcount,userdata->fps);
        ...
        if ( buffer->flags & MMAL_BUFFER_HEADER_FLAG_KEYFRAME) {
            config_keyframe=(uint8_t* )av_mallocz(buffer->length);
	    memcpy(config_keyframe,buffer->data,buffer->length);
	    config_keyframe_size=buffer->length;
        }
...
int setup_camera(PORT_USERDATA *userdata) {
    ...
    userdata->camera_video_port->buffer_num = 1;
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        if (userdata.overlay == 1) { 
            ...
            sprintf(text, "Golden Eye - %s  %.2fFPS ",time_stamp, userdata.fps);
