...
void saveImg(unsigned char *image, const char *index, bool AlarmSend) {
  ...
  if (AlarmSend) {
    ...
    int pid = fork();
    if (!pid) {
      execlp(MOTION_PATH_ALARM, MOTION_EXEC_ALARM, filesave, NULL);
    }
  }
}
...
static void Init_MP4() {
  ...
  if (!outputFormat) fprintf(stderr, "ERROR av guess format\n");
...
static void Close_MP4() {
  ...
  if (ret < 0) fprintf(stderr, "Error write trailer");

  if (AlertSendVideo) {
    signal(SIGCHLD, SIG_IGN);
    int pid = fork();
    if (!pid) {
      execlp(MOTION_PATH_VIDEO, MOTION_EXEC_VIDEO, filesave, NULL);
    }
  ...
  if (output_buffer) {
    ...
    if (userdata->detect >= there_is_motion) {
      ...
      if (count_exceeding > exceeding) {
        ...
        if (number_of_sequence > 0) count_exceeding++;
      }
    }

    if (number_of_sequence == 25 && count_exceeding == 0) {
      AlertSendVideo = true;
...
static void encoder_output_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
  ...
  if (buffer->flags & MMAL_BUFFER_HEADER_FLAG_CODECSIDEINFO) {
    ...
    // fprintf(stderr, " %d ", vcount);

    if (vcount >= there_is_motion) {
      detect_frame++;
      if (detect_frame > 3) {
        userdata->detect = vcount;
      }
    } else {
      if (detect_frame > 0) {
        detect_frame = 0;
    ...
    // fprintf(stderr, "vcount %d  frame %d  detect %d \n", vcount, detect_frame, userdata->detect);
