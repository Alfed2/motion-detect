...
Mat prev_frame, current_frame, next_frame, d1, d2, motion;
...
void saveImg(unsigned char* image, const char* index, bool AlarmSend ) {
   ...
   conv_yuv420_to_mat(imageMat, image, VIDEO_WIDTH, VIDEO_HEIGHT);
   snprintf(filesave,250,"%s%s%s.jpg",DirSave,timestrtemp,index);  
   imwrite(filesave, imageMat);
   resize(imageMat,imageMat,Size(),0.3,0.3);
   snprintf(filesave_small,250,"%s%s%s-small.jpg",DirSave,timestrtemp,index);
   imwrite(filesave_small, imageMat);
...
static void Init_MP4(){
    ...
    outputFormat = av_guess_format("mp4",filesave,NULL);
        ...
        outputStream->codecpar->format = AV_PIX_FMT_BGR24; //AV_PIX_FMT_YUV420P;
...
void camera_video_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
    ...
    if (output_buffer) {
    	...
    	conv_yuv420_to_mat(next_frame, pointer, VIDEO_WIDTH, VIDEO_HEIGHT); 
	...
	meanStdDev(motion, mean, stddev);
        ...
        fprintf(stderr, "1-ERROR: mmal_queue_get (%d)\n", output_buffer);
    }
    ...
    if (number_of_sequence==7 && count_exceeding==0 ) {
       AlertSendVideo = true;
       if (output_buffer) {
         unsigned char* pointer = (unsigned char *)(output_buffer->data);
...
int setup_camera(PORT_USERDATA *userdata) {
    ...
    format->encoding = MMAL_ENCODING_I420; //MMAL_ENCODING_BGR24;
    format->encoding_variant = MMAL_ENCODING_I420; //MMAL_ENCODING_BGR24;
    format->es->video.width = VIDEO_WIDTH;
    format->es->video.height = VIDEO_HEIGHT;
    format->es->video.crop.x = 0;
    format->es->video.crop.y = 0;
...
int main(int argc, char** argv) {
    ...
    prev_frame = Mat(Size(userdata.width,userdata.height), CV_8UC3);
    cvtColor(prev_frame, prev_frame, CV_RGB2GRAY);
    current_frame = next_frame =  prev_frame;
    ...
    while (1) {
        ...
        if (userdata.overlay == 1) { 
            ...
            sprintf(text, "Golden Eye - %s  %.1fFPS ",time_stamp, userdata.fps);
